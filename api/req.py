...
# Accept both "YYYY-MM-DD" and full ISO dates, combine with HH:MM into UTC ISO
def _combine_utc(date_str: str, hhmm: str | None) -> str | None:
    ...
    if "T" in s:
        s = s.split("T", 1)[0]  # keep date part only

    try:
        y, m, d = map(int, s.split("-"))
    except Exception:
        raise ValueError(f"Invalid date; expected 'YYYY-MM-DD' or ISO-8601, got '{date_str}'")
