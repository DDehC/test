...
// Allergy mapping between frontend and backend
...
const REVERSE_ALLERGY_MAPPING = Object.fromEntries(
  Object.entries(ALLERGY_MAPPING).map(([key, value]) => [value, key])
);

const ALLERGY_OPTIONS = Object.keys(ALLERGY_MAPPING);
...
export default function UserProfilePage() {
  ...
  const loadProfile = async () => {
    try {
      ...
      // Convert backend allergy string to frontend array
      const backendAllergy = user.allergy || "none"; // Fixed: was "allergy", should be "allergy"
      if (backendAllergy !== "none") {
        const frontendAllergy = REVERSE_ALLERGY_MAPPING[backendAllergy] || backendAllergy;
        setSelectedAllergies([frontendAllergy]);
      } else {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Convert frontend allergies to backend format
      const backendAllergy = selectedAllergies.length > 0 
        ? (ALLERGY_MAPPING[selectedAllergies[0]] || selectedAllergies[0].toLowerCase())
        : "none";

      const updateData = {
        email: profile.email,
        department: profile.department,
        allergy: backendAllergy, // Send as string to backend
      };
