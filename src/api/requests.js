...
export async function updateRequest(requestData) {
  const res = await fetch("/api/req/pubrequpdate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const publication = { ...data.publication, id: data.publication._id };
  return publication;
}

/* ---------- EVENT CALLERS ---------- */
async function create_event(id) {
  const res = await fetch(`${API}/req/eventcreate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
...
async function delete_event(source_request_id) {
  const res = await fetch(`${API}/req/eventdelete`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ source_request_id }),
  });
...
export const requestsApi = { list, update, remove, create_event, delete_event };
